    ...
    this.sesionTutoriaService.detallesByIdSesion(respReserva.idSesionTutoria!)
        .then(detallesExistentes =>{      
          ...
          if(detallesExistentes.length != 0){
            ...
            this.sesionTutoriaService.detalleTutoriaPUT(detalleSesion_Academico)
              .then(()=>{
                console.log("Actualizado con exito detalle academico");
              })
        
            this.sesionTutoriaService.detalleTutoriaPUT(detalleSesion_Profesional)
                .then(()=>{
                  console.log("Actualizado con exito detalle academico");
                })
            this.sesionTutoriaService.detalleTutoriaPUT(detalleSesion_Personal)
                .then(()=>{
                  this.messageService.add({ severity: 'success', summary: 'Success', detail: 'La sesion de tutoría fue actualizado con exito.' });
                  //console.log("Actualizado con exito detalle academico");
                })
          }else{
            // Crear objetos de tipo detalle sesion                          
            ...
            detalleSesion_Personal.observaciones = DPersonal_observacion;
            //Guardar objetos de tipo detalle sesiom
            this.sesionTutoriaService.detalleTutoriaPOST(detalleSesion_Academico)
                .then(()=>{
                  //this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Datos del tutorado fueron actualizados con exito!' });
                  console.log("Guadado con exito detalle academico");
                })
        
            this.sesionTutoriaService.detalleTutoriaPOST(detalleSesion_Profesional)
                .then(()=>{
                  console.log("Guadado con exito detalle profesional");
                })
            this.sesionTutoriaService.detalleTutoriaPOST(detalleSesion_Personal)
                .then(()=>{
                  this.messageService.add({ severity: 'success', summary: 'Success', detail: 'La sesion de tutoría fue registrado con exito.' });
                  console.log("Guadado con exito detalle detallepersonal");
                })
          }
        })
  }
